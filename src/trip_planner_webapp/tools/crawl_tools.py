...
# ---------------------------------------------------------------------------
# Rentalcars search (via Serper web search)
# ---------------------------------------------------------------------------

@tool("Rentalcars search")
def search_rentalcars(
    location: str,
    pickup_date: str,
    return_date: str,
) -> str:
    """Busca alquiler de coches en Rentalcars.com buscando precios reales en Google.
    Devuelve resultados de búsqueda web con precios y un enlace a Rentalcars.

    Args:
        location: Ciudad o aeropuerto (ej: 'Keflavik Airport' o 'Reykjavik')
        pickup_date: Fecha recogida YYYY-MM-DD
        return_date: Fecha devolución YYYY-MM-DD
    """
    cache_key = f"rentalcars_{location}_{pickup_date}_{return_date}"
    cached = _cache_get("crawl_cars", cache_key)
    if cached:
        return cached

    from .search_tools import web_search_es

    query1 = f"rentalcars {location} alquiler coche {pickup_date} {return_date} precio"
    query2 = f"site:rentalcars.com {location} alquiler coche"
    results1 = web_search_es.run(search_query=query1)
    results2 = web_search_es.run(search_query=query2)

    rentalcars_url = f"https://www.rentalcars.com/es/?location={location.replace(' ', '+')}"

    result = json.dumps({
        "source": "rentalcars.com",
        "url": rentalcars_url,
        "search_results": results1,
        "rentalcars_listings": results2,
        "note": "Precios de búsqueda web real de Rentalcars. Usa SOLO estos precios.",
    }, ensure_ascii=False)
...
@tool("PDF content extract")
def extract_pdf_content(file_path: str) -> str:
    """Extrae texto de un archivo PDF local (guías de viaje, folletos, itinerarios).
    Devuelve el contenido textual del PDF en formato JSON.

    Args:
        file_path: Ruta absoluta del archivo PDF en el servidor
    """
    try:
        import fitz
